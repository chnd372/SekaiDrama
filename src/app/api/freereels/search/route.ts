...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await cachedFetch("src/app/api/freereels/search/route.ts", `${process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.sansekai.my.id/api"}/freereels/search?query=${encodeURIComponent(query)}`, {
      method: "GET",
      headers: {
